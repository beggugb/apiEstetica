  ...
  static servicios(req,res) {
    return new Promise((resolve, reject) => {
        upload(req, res, function (err) {
          ...
          sharp(req.file.path).resize({ height: 300 }).toFile('./api/public/images/servicios/lg/' + req.file.filename);
          sharp(req.file.path).resize({ height: 150 }).toFile('./api/public/images/servicios/md/' + req.file.filename);
          sharp(req.file.path).resize({ height: 75 }).toFile('./api/public/images/servicios/sm/' + req.file.filename);
          resolve(req.file)
        })
      })
  }
